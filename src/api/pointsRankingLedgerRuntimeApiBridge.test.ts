...
import type { PointsRankingLedgerRuntime } from "../domain/types";
import {
  buildPointsRankingLedgerRuntimeApiUrl,
  ...
  type PointsRankingLedgerRuntimeApiFetch,
} from "./pointsRankingLedgerRuntimeApiBridge";
...
const validRuntimePayload = (
  overrides: Partial<PointsRankingLedgerRuntime> = {},
): PointsRankingLedgerRuntime => ({
  ...createPointsRankingLedgerRuntimeApiSeededFallbackState(campaignId).runtime,
  source: "api_runtime",
...
const envelope = (payload: unknown, traceId = "trace-runtime-envelope") => ({
  data: {
    boundary: {
      "en-US": "Local points/ranking ledger runtime review route.",
      "zh-CN": "本地 points/ranking ledger runtime review route。",
      "zh-TW": "Local points/ranking ledger runtime review route.",
    },
    payload,
  },
  ok: true,
...
describe("points ranking ledger runtime API bridge", () => {
  it("creates a loading state without touching the network", () => {
    const state = createPointsRankingLedgerRuntimeApiLoadingState(campaignId);
    ...
    expect(state.boundary["en-US"]).toContain("Local points ranking ledger runtime API bridge");
    expect(state.runtime.noLiveSideEffects.liveBackendLedgerWrite).toBe(false);
  });

  it("returns seeded fallback when the API base URL is missing", async () => {
    const fetchImpl = vi.fn() as unknown as PointsRankingLedgerRuntimeApiFetch;

    const state = await loadPointsRankingLedgerRuntimeApiBridgeState({
      campaignId,
      config: { baseUrl: "   " },
      fetchImpl,
    });
    ...
    expect(state.runtime.source).toBe("fallback");
    expect(state.runtime.eligibilityRoot.contractRootMode).toBe("none");
  });
  ...
  it("returns sanitized error fallback when the API base URL is invalid", async () => {
    const state = await loadPointsRankingLedgerRuntimeApiBridgeState({
      ...
      fetchImpl: vi.fn() as unknown as PointsRankingLedgerRuntimeApiFetch,
    });
  ...
  it("loads runtime payload from a configured API with trace id and route count", async () => {
    const fetchImpl = vi.fn().mockResolvedValueOnce(response(
      envelope(validRuntimePayload({ status: "review_required" }), "trace-envelope"),
      { traceId: "trace-header" },
    )) as unknown as PointsRankingLedgerRuntimeApiFetch;
    ...
    expect(state.runtime.ledger.events[0]).toEqual(expect.objectContaining({
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "http://127.0.0.1:5174/api/campaigns/camp-awaken-sprint/points-ranking-ledger-runtime",
      expect.objectContaining({
        headers: expect.objectContaining({
          accept: "application/json",
          "x-campaign-os-roles": "project_owner",
          "x-campaign-os-trace-id": expect.stringMatching(/^points-runtime-points-ranking-ledger-runtime-/),
        }),
        method: "GET",
      }),
    );
  ...
  it("builds the request URL without leaking query strings", () => {
    ...
    ).toBe("http://127.0.0.1:5174/base/api/campaigns/campaign%2Fwith%20space/points-ranking-ledger-runtime");
  });
  ...
  it("returns error fallback for malformed envelope", async () => {
    const fetchImpl = vi.fn().mockResolvedValueOnce(response({
      data: { payload: validRuntimePayload() },
      runtime: runtimeMetadata,
  ...
  it("returns error fallback when required payload sections are missing", async () => {
    const { ledger: _ledger, ...invalidPayload } = validRuntimePayload();
    const fetchImpl = vi.fn().mockResolvedValueOnce(response(
  ...
  it("rejects payloads where no-live flags are not all false", async () => {
    const invalidPayload = validRuntimePayload({
      noLiveSideEffects: {
        ...validRuntimePayload().noLiveSideEffects,
        liveBackendLedgerWrite: true as false,
      },
    });
    const fetchImpl = vi.fn().mockResolvedValueOnce(response(
  ...
  it("redacts unsafe diagnostic text from failed envelopes", async () => {
    const fetchImpl = vi.fn().mockResolvedValueOnce(response({
      error: {
        message:
          "private key, seed phrase, bearer abc.123, signed URL, object key, storage key, provider call, wallet signature, raw signature at handler (/private/path.ts:1)",
      },
      ok: false,
  ...
  it("sanitizes standalone diagnostic text", () => {
    expect(
      sanitizePointsRankingLedgerRuntimeApiText("provider payload token=abc123 private key signed url object key"),

...
import {
  buildObjectStorageExportRuntimeApiUrl,
  ...
  type ObjectStorageExportReadinessPayload,
  type ObjectStorageExportRuntimeApiFetch,
} from "./objectStorageExportRuntimeApiBridge";
...
const validReadinessPayload = (
  overrides: Partial<ObjectStorageExportReadinessPayload> = {},
): ObjectStorageExportReadinessPayload => ({
  ...createObjectStorageExportRuntimeApiSeededFallbackState(campaignId, "trace-seeded").readiness,
  source: "api_runtime",
  traceId: "trace-storage-payload",
  ...overrides,
});

const envelope = (payload: unknown, traceId = "trace-storage-envelope") => ({
  data: {
    boundary: {
      "en-US": "Local object storage export readiness review route.",
      "zh-CN": "本地 object storage export readiness review route。",
      "zh-TW": "Local object storage export readiness review route.",
    },
    payload,
  },
  ok: true,
...
describe("object storage export runtime API bridge", () => {
  it("creates a loading state without touching the network", () => {
    const state = createObjectStorageExportRuntimeApiLoadingState(campaignId);
    ...
    expect(state.boundary["en-US"]).toContain("Local object storage export readiness API bridge");
    expect(state.readiness.safety.liveUploadEnabled).toBe(false);
    expect(state.readiness.safety.signedUrlCreated).toBe(false);
  });

  it("returns seeded fallback when the API base URL is missing", async () => {
    const fetchImpl = vi.fn() as unknown as ObjectStorageExportRuntimeApiFetch;

    const state = await loadObjectStorageExportRuntimeApiBridgeState({
      campaignId,
      config: { baseUrl: "   " },
      fetchImpl,
    });
    ...
    expect(state.readiness.manifestSummary.containsSignedUrl).toBe(false);
  });
  ...
  it("returns sanitized error fallback when the API base URL is invalid", async () => {
    const state = await loadObjectStorageExportRuntimeApiBridgeState({
      ...
      fetchImpl: vi.fn() as unknown as ObjectStorageExportRuntimeApiFetch,
    });
  ...
  it("loads readiness payload from a configured API with trace id and route count", async () => {
    ...
    )) as unknown as ObjectStorageExportRuntimeApiFetch;
    ...
    expect(state.readiness.safety).toMatchObject({
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "http://127.0.0.1:5174/api/campaigns/camp-awaken-sprint/export/storage-readiness",
      expect.objectContaining({
        headers: expect.objectContaining({
          accept: "application/json",
          "x-campaign-os-roles": "project_owner",
          "x-campaign-os-trace-id": expect.stringMatching(/^storage-runtime-object-storage-export-runtime-/),
        }),
        method: "GET",
      }),
    );
  ...
  it("maps local-ready payloads to review-required UI state", async () => {
    const fetchImpl = vi.fn().mockResolvedValueOnce(response(
      envelope(validReadinessPayload({
        providerStatus: "ready_for_provider_binding",
        status: "local_ready",
      }), "trace-local-ready"),
    )) as unknown as ObjectStorageExportRuntimeApiFetch;
  ...
  it("builds the request URL without leaking query strings", () => {
    ...
    ).toBe("http://127.0.0.1:5174/base/api/campaigns/campaign%2Fwith%20space/export/storage-readiness");
  });
  ...
  it("returns error fallback when required payload sections are missing", async () => {
    const { safety: _safety, ...invalidPayload } = validReadinessPayload();
    const fetchImpl = vi.fn().mockResolvedValueOnce(response(
  ...
  it("rejects payloads where no-live flags are enabled", async () => {
    const invalidPayload = validReadinessPayload({
      safety: {
        ...validReadinessPayload().safety,
        signedUrlCreated: true as false,
      },
    });
    const fetchImpl = vi.fn().mockResolvedValueOnce(response(
  ...
  it("redacts unsafe diagnostic text from failed envelopes", async () => {
    const fetchImpl = vi.fn().mockResolvedValueOnce(response({
      error: {
        message:
          "private key, seed phrase, bearer abc.123, signed URL, download URL, object key, storage key, provider call, wallet signature, raw signature at handler (/private/path.ts:1)",
      },
      ok: false,
  ...
  it("sanitizes standalone diagnostic text", () => {
    expect(
      sanitizeObjectStorageExportRuntimeApiText("provider payload token=abc123 private key signed url object key"),

...
import {
  buildContractWriterRuntimeApiUrl,
  ...
  type ContractWriterRuntimeApiFetch,
} from "./contractWriterRuntimeApiBridge";
import {
...
const validReadinessPayload = (
  overrides: Partial<ContractWriterRuntimeReadiness> = {},
): ContractWriterRuntimeReadiness => ({
  ...createContractWriterRuntimeReadiness({
    campaign: campaignDetail,
    source: "server_runtime",
    traceId: "trace-contract-writer-payload",
  }),
  ...overrides,
});

const envelope = (payload: unknown, traceId = "trace-contract-writer-envelope") => ({
  data: {
    boundary: {
      "en-US": "Local contract writer readiness review route.",
      "zh-CN": "本地 contract writer readiness review route。",
      "zh-TW": "Local contract writer readiness review route.",
    },
    payload,
  },
  ok: true,
...
describe("contract writer runtime API bridge", () => {
  it("creates a loading state without touching the network", () => {
    const state = createContractWriterRuntimeApiLoadingState(campaignId);
    ...
    expect(state.boundary["en-US"]).toContain("Local contract writer runtime API bridge");
    expect(state.readiness.noLiveSideEffects.liveSignerExecution).toBe(false);
    expect(state.readiness.noLiveSideEffects.liveContractWrite).toBe(false);
  });

  it("returns seeded fallback when the API base URL is missing", async () => {
    const fetchImpl = vi.fn() as unknown as ContractWriterRuntimeApiFetch;

    const state = await loadContractWriterRuntimeApiBridgeState({
      campaignId,
      config: { baseUrl: "   " },
      fetchImpl,
    });
    ...
    expect(state.readiness.configHandoff.productionReady).toBe(false);
  });
  ...
  it("returns sanitized error fallback when the API base URL is invalid", async () => {
    const state = await loadContractWriterRuntimeApiBridgeState({
      ...
      fetchImpl: vi.fn() as unknown as ContractWriterRuntimeApiFetch,
    });
  ...
  it("loads readiness payload from a configured API with trace id and route count", async () => {
    ...
    )) as unknown as ContractWriterRuntimeApiFetch;
    ...
    expect(state.readiness.operationCatalog).toHaveLength(4);
    expect(state.readiness.summary.operationCount).toBe(20);
    expect(fetchImpl).toHaveBeenCalledWith(
      "http://127.0.0.1:5174/api/campaigns/camp-awaken-sprint/contract-writer/readiness",
      expect.objectContaining({
        headers: expect.objectContaining({
          accept: "application/json",
          "x-campaign-os-roles": "project_owner",
          "x-campaign-os-trace-id": expect.stringMatching(/^contract-runtime-contract-writer-runtime-/),
        }),
        method: "GET",
      }),
    );
  ...
  it("builds the request URL without leaking query strings", () => {
    ...
    ).toBe("http://127.0.0.1:5174/base/api/campaigns/campaign%2Fwith%20space/contract-writer/readiness");
  });
  ...
  it("rejects payloads where no-live flags are enabled", async () => {
    const invalidPayload = validReadinessPayload({
      noLiveSideEffects: {
        ...
        liveContractWrite: true as false,
      },
    });
    const fetchImpl = vi.fn().mockResolvedValueOnce(response(
  ...
  it("sanitizes standalone diagnostic text", () => {
    expect(
      sanitizeContractWriterRuntimeApiText("provider payload token=abc123 private key signed url object key"),
    ).toBe("[REDACTED:PROVIDER_PAYLOAD] [REDACTED:CREDENTIAL] [REDACTED:PRIVATE_KEY] [REDACTED:SIGNED_URL] [REDACTED:OBJECT_KEY]");
  });

  it("creates an explicit seeded fallback state for disabled Project Console reviews", () => {
    const state = createContractWriterRuntimeApiSeededFallbackState(campaignId, "trace-seeded");

...
export const backendServiceBoundaries = [
  service({
    ...
    description: "Campaign lifecycle, publish checklist, launch readiness, delivery readiness, publish delivery joint review, wallet policy, locale, contract mode, companion contract readiness, contract writer runtime readiness, reward distribution handoff readiness, and contract transparency boundary.",
    domainArea: "campaign",
    ...
    risks: [
      ...
      "Delivery readiness, publish delivery joint review, contract writer readiness, reward distribution handoff readiness, and contract transparency routes are read-only local review metadata and do not enable wallet SDK, signer execution, contract writes, storage, reward custody, or reward distribution capabilities.",
    ],
    routeIds: [
      "campaigns.list",
  ...
  service({
    ...
    deploymentUnit: "api-runtime",
    description: "Normalized AA/EOA wallet session boundary with repository-backed local wallet_sessions persistence.",
    domainArea: "wallet",
    ...
    productionRequired: true,
    readiness: "local_only",
    risks: ["Repository-backed wallet session records are local review only; live wallet verification, production session storage, signing keys, secret manager, and RBAC are deferred."],
  ...
  service({
    ...
    description: "Task evidence verification and provider readiness boundary across chain, dApp, social, and manual sources.",
  ...
  service({
    ...
    deploymentUnit: "api-runtime",
    description: "Wallet-aware referral relationship boundary backed by the local Campaign DB referral binding read model.",
    domainArea: "referral",
    ...
    readiness: "local_only",
    risks: [
      "Campaign referral binding read model is local deterministic or durable-test only; production Campaign DB referral binding table migration is deferred.",
      "Live wallet verification, provider risk signals, contract transactions, reward custody, and reward distribution are disabled.",
      "Referral risk event ingestion and scoring handoff require the deferred risk store, provider handoff, scheduler runtime, queue runtime, worker lease, idempotency store, and dead-letter handling before production.",
    ],
    routeIds: [],
    runtimeProfiles: productionProfiles,
  }),
...
export const backendDataStores = [
  dataStore({
    ...
    records: [
      "campaigns",
      "campaign_tasks",
      "campaign_publish_checks",
      "campaign_participants",
      "campaign_referral_bindings",
      "referral_rules",
    ],
    retentionRisk: "Campaign configuration, task records, participant wallet/risk read models, referral binding relationships, and retention policy need explicit production table rules before production.",
  }),
  dataStore({
    containsSensitiveData: true,
    ...
    productionMode: "relational_db",
    records: ["wallet_sessions", "account_type_evidence", "session_audit", "repository_health"],
    retentionRisk: "Wallet session repository metadata needs privacy, expiry, and production session store rules before production.",
  }),
  dataStore({
    containsSensitiveData: true,

...
const backendSurfaceReadiness = [
  ...
  },
  {
    ...
    label: "Verification",
    notes: "Local seeded verification exists; provider/indexer handoff, provider evidence, queue runtime, queue provider adapter activation, queue provider driver SDK binding, scheduler runtime, retry/backoff, worker idempotency store readiness metadata, worker lease store metadata, and dead-letter handling are deferred with pending/manual_review degradation.",
  ...
  },
  {
    ...
    label: "Analytics",
    notes: "Local analytics summary exists; warehouse-backed analytics ingestion, scheduler runtime, queue runtime, queue provider adapter activation, queue provider driver SDK binding, dead-letter queue, worker idempotency store readiness metadata, and observability exporter are deferred.",
  ...
  },
  {
    deferredDependencies: ["production_database", "provider_adapters", "scheduler", "worker_queue"],
    label: "Risk Scoring",
    notes: "Topology exists; analytics warehouse adapter ingestion, stale review cleanup scheduler runtime, queue runtime, queue provider adapter activation, queue provider driver SDK binding, worker idempotency store readiness metadata, worker lease, dead-letter handling, and scoring workers are deferred.",
    routeIds: [],
  ...
  },
  {
    ...
    label: "AI Ops",
    notes: "Topology exists; AI provider adapter calls, approval flow, scheduler runtime, queue runtime, queue provider adapter activation, queue provider driver SDK binding, worker idempotency store readiness metadata, observability exporter, and worker execution are deferred.",
    routeIds: [],
  ...
  },
  {
    ...
    notes: "Local service readiness metadata exists; provider registry resolution, queue provider adapter readiness, queue provider driver readiness, worker idempotency store readiness metadata, and live adapter activation are deferred.",
  ...
  },
  {
    ...
    notes: "Local health, contracts, route coverage, topology reporting, queue provider driver readiness metadata, worker idempotency store readiness metadata, worker lease readiness metadata, and observability exporter readiness metadata exist; live telemetry export, metrics sink writes, structured logs, traces, alerts, and retry/dead-letter delivery remain disabled while contract sync handoff depends on deferred scheduler runtime, queue runtime, queue provider adapter readiness, queue provider driver SDK binding, worker idempotency store activation, worker lease store activation, dead-letter queue, and observability exporter.",

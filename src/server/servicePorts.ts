...
export const apiServicePorts = [
  servicePort({
    ...
    localAdapter: "src/server/handlers.ts runtime health/contracts and production database handoff handlers",
    notes: "Local runtime, contract metadata, production database handoff readiness, analytics summary, analytics ingestion readiness, and observability exporter readiness are available; live database connections, DB client construction, queries, writes, transactions, migration execution, secret reveal, live analytics SDK execution, warehouse writes, telemetry export, metrics sink writes, logs, traces, alerts, and retry/dead-letter delivery remain disabled.",
  ...
  servicePort({
    deferredCapabilities: ["auth_session", "contract_writer", "production_database"],
    futureAttachPoints: [
      "campaign repository",
      ...
      "reward distribution handoff readiness review",
    ...
    id: "campaign-port",
    localAdapter: "src/domain/campaignService.ts campaign local handlers with src/server/contractWriterRuntime.ts readiness adapter",
    notes: "Campaign discovery, draft creation, lifecycle operation inspection, launch readiness inspection, delivery readiness inspection, publish delivery joint review bridge inspection, companion contract readiness inspection, contract writer runtime readiness inspection, reward distribution handoff readiness inspection, contract transparency inspection, campaign participant repository/read model metadata, and campaign referral binding read model metadata use local deterministic or durable-test read models; production DB migration, mutation audit, scheduler handoff, auth, wallet SDK calls, provider risk calls, signer execution, contract writes, storage writes, reward custody, and reward distribution are deferred.",
    productionAdapterStatus: "local_seeded",
    requiresExternalNetwork: false,
    requiresSecret: false,
    routeIds: [

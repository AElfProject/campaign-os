...
const unsupportedLocaleError: I18nReviewActionError = {
  ...
  field: "targetLocale",
  message: localized(
    "Only zh-CN, zh-TW, ja-JP, ko-KR, and vi-VN can be target locales for this local review workflow.",
    "该本地审核工作流仅支持 zh-CN、zh-TW、ja-JP、ko-KR 与 vi-VN 作为目标语言。",
    "Only zh-CN, zh-TW, ja-JP, ko-KR, and vi-VN can be target locales for this local review workflow.",
  ),
};
...
const localeLabels: Record<ContentRevision["locale"], LocalizedText> = {
  ...
  "vi-VN": {
    "en-US": "Vietnamese fallback",
    "zh-CN": "越南语英文回退",
    "zh-TW": "Vietnamese fallback",
  },
  "id-ID": {
    "en-US": "Indonesian fallback",
    "zh-CN": "印尼语英文回退",
    "zh-TW": "Indonesian fallback",
  },
  "tr-TR": {
    "en-US": "Turkish fallback",
    "zh-CN": "土耳其语英文回退",
    "zh-TW": "Turkish fallback",
  },
  "es-ES": {
    "en-US": "Spanish fallback",
    "zh-CN": "西班牙语英文回退",
    "zh-TW": "Spanish fallback",
  },
};
...
const createStateComponentFamilies = (): StateComponentFamily[] => [
  ...
  stateComponentFamily({
    ...
    examples: [
      stateComponentExample({
        id: "task-ready",
        label: localized("Ready", "可开始"),
        meaning: localized("Task can be started or verified from the local preview.", "任务可从本地预览开始或验证。"),
        userMessage: localized("Start bridge, swap, or wallet task when prerequisites are met.", "满足前置条件后开始 bridge、swap 或钱包任务。"),
        nextAction: localized("Open the relevant ecosystem task step.", "打开对应生态任务步骤。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "v0.1 task status sequence",
      }),
      stateComponentExample({
        id: "task-pending-verification",
        label: localized("Pending Verification", "等待验证"),
        meaning: localized("Evidence is waiting for local provider/readiness review.", "证据等待本地 provider/readiness 审核。"),
        userMessage: localized("Waiting for chain confirmation or operator review.", "等待链上确认或运营审核。"),
        nextAction: localized("Check evidence source and retry verification if needed.", "检查证据来源，必要时重新验证。"),
        ownerRole: "internal_operator",
        readiness: "review_required",
        sourceReference: "v0.1 task status sequence",
      }),
      stateComponentExample({
        id: "task-failed",
        label: localized("Failed", "失败"),
        meaning: localized("The task did not meet the required verification condition.", "任务未满足所需验证条件。"),
        userMessage: localized("Verification failed. Complete bridge or swap again, then retry verification.", "验证失败。请重新完成 bridge 或 swap 后再次验证。"),
        nextAction: localized("Retry verification, go to bridge, complete swap, or contact the project owner.", "重新验证、前往 bridge、完成 swap，或联系项目方。"),
        ownerRole: "project_owner",
        readiness: "review_required",
        sourceReference: "v0.1 task status sequence",
      }),
      stateComponentExample({
        ...
        meaning: localized("Task requires human judgment before points or eligibility are trusted.", "任务需要人工判断后才能信任积分或资格。"),
        userMessage: localized("Manual review is required for this task.", "该任务需要人工审核。"),
        nextAction: localized("Submit proof and route the case to operator review.", "提交证明并转交运营审核。"),
        ownerRole: "internal_operator",
        readiness: "review_required",
        sourceReference: "v0.1 task status sequence",
      }),
  ...
  stateComponentFamily({
    ...
    examples: [
      stateComponentExample({
        ...
        label: localized("Eligible", "符合资格"),
        meaning: localized("Required tasks and review inputs are complete.", "必需任务与审核输入已完成。"),
        userMessage: localized("Ready for winner list review.", "可进入 winner list 审核。"),
        nextAction: localized("Review export readiness before reward fulfillment.", "奖励履约前审核导出准备度。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "v0.1 eligibility checker",
      }),
      stateComponentExample({
        ...
        label: localized("Not eligible", "暂不符合资格"),
        meaning: localized("Participant is missing required tasks or points.", "参与者缺少必需任务或积分。"),
        userMessage: localized("Missing: bridge via eBridge and swap on Awaken.", "缺少：通过 eBridge bridge，并在 Awaken swap。"),
        nextAction: localized("Fix missing tasks and re-check eligibility.", "补齐缺失任务并重新检查资格。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "v0.1 eligibility checker",
      }),
      stateComponentExample({
        ...
        label: localized("Pending", "待确认"),
        meaning: localized("Eligibility depends on pending verification or manual review.", "资格取决于待完成验证或人工审核。"),
        userMessage: localized("Some task evidence is still pending.", "部分任务证据仍在等待确认。"),
        nextAction: localized("Wait for verification or ask the operator to review evidence.", "等待验证，或请运营审核证据。"),
        ownerRole: "internal_operator",
        readiness: "review_required",
        sourceReference: "v0.1 eligibility checker",
      }),
      stateComponentExample({
        id: "eligibility-risk-flagged",
        label: localized("Risk flagged", "风险标记"),
        meaning: localized("Eligibility must be reviewed because risk signals are present.", "因存在风险信号，资格需要审核。"),
        userMessage: localized("Manual risk review required before winner export.", "winner 导出前需要人工风险审核。"),
        nextAction: localized("Review risk reason without exposing internal review rules.", "审核风险原因，但不暴露内部审核规则。"),
        ownerRole: "internal_operator",
        readiness: "blocked",
        sourceReference: "Admin risk review state",
      }),
    ],
  }),
  stateComponentFamily({
    id: "i18n_content",
    label: localized("i18n Content", "多语言内容"),
    description: localized(
      "Visible locale states for source, drafts, review, publication, and fallback.",
      "展示源内容、草稿、审核、发布与 fallback 的语言状态。",
    ),
    ownerRole: "project_owner",
    sourceReference: "v0.2 interaction design i18n states",
    examples: [
      stateComponentExample({
        id: "i18n-missing",
        label: localized("Missing", "缺失"),
        meaning: localized("A locale has no approved localized content yet.", "某语言暂无已批准的本地化内容。"),
        userMessage: localized("Localized content is missing and may use English fallback.", "本地化内容缺失，可能使用英文 fallback。"),
        nextAction: localized("Generate or write the missing locale draft.", "生成或编写缺失语言草稿。"),
        ownerRole: "project_owner",
        readiness: "review_required",
        sourceReference: "v0.2 i18n state requirements",
      }),
      stateComponentExample({
        id: "i18n-ai-draft",
        label: localized("AI Draft", "AI 草稿"),
        meaning: localized("AI generated copy exists but cannot auto-publish.", "AI 生成内容已存在，但不能自动发布。"),
        userMessage: localized("Chinese AI draft needs human review before publish.", "中文 AI 草稿发布前需要人工审核。"),
        nextAction: localized("Compare with English and mark reviewed.", "与英文对照并标记为已审核。"),
        ownerRole: "project_owner",
        readiness: "review_required",
        sourceReference: "v0.2 i18n state requirements",
      }),
      stateComponentExample({
        id: "i18n-reviewed",
        label: localized("Reviewed", "已审核"),
        meaning: localized("Human reviewer has checked the localized copy.", "人工审核人已检查本地化文案。"),
        userMessage: localized("Translation reviewed and ready for publish decision.", "翻译已审核，可进入发布决策。"),
        nextAction: localized("Publish revision after reward disclaimer review.", "奖励免责声明审核后发布修订。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "Translation Manager",
      }),
      stateComponentExample({
        id: "i18n-published",
        label: localized("Published", "已发布"),
        meaning: localized("Locale content is live in the local content contract.", "语言内容已在本地内容 contract 中发布。"),
        userMessage: localized("English source content is published.", "英文源内容已发布。"),
        nextAction: localized("Monitor fallback and edit history.", "监控 fallback 和编辑历史。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "Translation Manager",
      }),
      stateComponentExample({
        id: "i18n-fallback",
        label: localized("Fallback", "Fallback"),
        meaning: localized("Unsupported or unreviewed locale uses the English fallback.", "不支持或未审核语言使用英文 fallback。"),
        userMessage: localized("Traditional Chinese falls back to English until reviewed.", "繁中在审核前回退展示英文。"),
        nextAction: localized("Review fallback copy before treating it as final localization.", "将 fallback 视作最终本地化前先审核文案。"),
        ownerRole: "project_owner",
        readiness: "review_required",
        sourceReference: "v0.2 i18n state requirements",
      }),
  ...
  stateComponentFamily({
    id: "wallet_qa",
    label: localized("Wallet/QA", "钱包/QA"),
    description: localized(
      "Wallet connection and QA states for AA, EOA, chain, and signature readiness.",
      "面向 AA、EOA、链与签名 readiness 的钱包连接和 QA 状态。",
    ),
    ownerRole: "internal_operator",
    sourceReference: "Wallet connection diagnostics",
    examples: [
      stateComponentExample({
        id: "wallet-connect-ready",
        label: localized("Connect ready", "连接就绪"),
        meaning: localized("Supported wallet can enter the local campaign flow.", "受支持钱包可进入本地活动流程。"),
        userMessage: localized("Connect Portkey AA or supported EOA wallet.", "连接 Portkey AA 或受支持 EOA 钱包。"),
        nextAction: localized("Open wallet connect modal.", "打开钱包连接弹窗。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "Wallet connect modal flow",
      }),
      stateComponentExample({
        id: "wallet-wrong-chain",
        label: localized("Wrong chain", "链错误"),
        meaning: localized("Wallet is connected to a chain that cannot verify this campaign.", "钱包连接到无法验证该活动的链。"),
        userMessage: localized("Switch to aelf network before verifying tasks.", "验证任务前请切换到 aelf 网络。"),
        nextAction: localized("Switch network or reconnect wallet.", "切换网络或重新连接钱包。"),
        ownerRole: "project_owner",
        readiness: "review_required",
        sourceReference: "Wallet diagnostics",
      }),
      stateComponentExample({
        id: "wallet-missing-signing",
        label: localized("Missing signing", "缺少签名"),
        meaning: localized("Wallet proof is missing for flows that require owner confirmation.", "需要用户确认的流程缺少钱包证明。"),
        userMessage: localized("Wallet confirmation is required before task verification can be trusted.", "任务验证可信前需要钱包确认。"),
        nextAction: localized("Reconnect wallet and complete confirmation.", "重新连接钱包并完成确认。"),
        ownerRole: "project_owner",
        readiness: "blocked",
        sourceReference: "Wallet diagnostics",
      }),
      stateComponentExample({
        ...
        label: localized("Unsupported wallet", "不支持的钱包"),
        meaning: localized("Wallet type is outside the campaign wallet policy.", "钱包类型不符合活动钱包策略。"),
        userMessage: localized("This campaign supports Portkey AA and supported EOA wallets.", "该活动支持 Portkey AA 与受支持 EOA 钱包。"),
        nextAction: localized("Connect a supported wallet or contact the project owner.", "连接受支持钱包，或联系项目方。"),
        ownerRole: "project_owner",
        readiness: "blocked",
        sourceReference: "Wallet diagnostics",
      }),
  ...
  stateComponentFamily({
    id: "export_modal",
    label: localized("Export/Modal", "导出/弹窗"),
    description: localized(
      "Export preview and modal states clarify review gates and reward responsibility.",
      "导出预览与弹窗状态明确审核门禁和奖励责任。",
    ),
    ownerRole: "project_owner",
    sourceReference: "v0.1 analytics export modal",
    examples: [
      stateComponentExample({
        id: "export-preview",
        label: localized("Export preview", "导出预览"),
        meaning: localized("Local winner rows are available for review.", "本地 winner 行可供审核。"),
        userMessage: localized("CSV/JSON preview is local review data only.", "CSV/JSON 预览仅为本地审核数据。"),
        nextAction: localized("Review required columns, evidence, risk flags, and reward responsibility.", "审核必需字段、证据、风险标记和奖励责任。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "Analytics export preview",
      }),
      stateComponentExample({
        id: "export-modal-review",
        label: localized("Export modal review", "导出弹窗审核"),
        meaning: localized("Export intent must show that exporting does not distribute rewards.", "导出意图必须说明导出不等于发奖。"),
        userMessage: localized("Export winners does not distribute rewards. Final rewards are handled by the campaign project.", "导出 winners 不等于发奖。最终奖励由活动项目方处理。"),
        nextAction: localized("Acknowledge project-owned reward distribution before export handoff.", "导出 handoff 前确认奖励由项目方负责。"),
        ownerRole: "project_owner",
        readiness: "review_required",
        sourceReference: "v0.1 export winners modal",
      }),
      stateComponentExample({
        id: "export-blocked",
        label: localized("Export blocked", "导出阻断"),
        meaning: localized("Export cannot proceed while required fields or risk review are missing.", "必需字段或风险审核缺失时不能继续导出。"),
        userMessage: localized("Export blocked until required fields and review gates pass.", "必需字段和审核门禁通过前导出被阻断。"),
        nextAction: localized("Resolve missing export fields and risk review before any handoff.", "任何 handoff 前先处理缺失导出字段和风险审核。"),
        ownerRole: "internal_operator",
        readiness: "blocked",
        sourceReference: "Export confirmation readiness",
      }),
    ],
  }),
  stateComponentFamily({
    id: "toast_notification",
    label: localized("Toast/Notification", "Toast/通知"),
    description: localized(
      "Notification states show completion, failure, and review feedback.",
      "通知状态展示完成、失败与审核反馈。",
    ),
    ownerRole: "internal_operator",
    sourceReference: "v0.1 state delivery screen 15",
    examples: [
      stateComponentExample({
        id: "toast-loading",
        label: localized("Loading", "加载中"),
        meaning: localized("A seeded/local operation is still preparing its review payload.", "seeded/本地操作仍在准备审核 payload。"),
        userMessage: localized("Loading local review data. No live sync is running.", "正在加载本地审核数据，不会执行实时同步。"),
        nextAction: localized("Wait for the local preview to finish, then inspect the recovered state.", "等待本地预览完成，然后检查恢复后的状态。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "v0.1 state delivery loading baseline",
      }),
      stateComponentExample({
        id: "toast-empty",
        label: localized("Empty", "空状态"),
        meaning: localized("No local rows match the selected review filter.", "当前审核筛选条件下没有本地行。"),
        userMessage: localized("No matching review items yet. Adjust filters or continue campaign setup.", "暂无匹配的审核项。可调整筛选或继续配置活动。"),
        nextAction: localized("Clear filters, add seeded participants, or complete the required setup step.", "清除筛选、添加 seeded 参与者，或完成必需配置步骤。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "v0.1 state delivery empty baseline",
      }),
      stateComponentExample({
        id: "toast-error",
        label: localized("Error", "错误"),
        meaning: localized("A local-only preview failed and must show a recoverable next step.", "本地预览失败时必须展示可恢复的下一步。"),
        userMessage: localized("Local preview failed. No live operation was executed.", "本地预览失败。未执行任何实时操作。"),
        nextAction: localized("Retry the local preview, check seeded data, or contact the project owner.", "重试本地预览、检查 seeded 数据，或联系项目方。"),
        ownerRole: "internal_operator",
        readiness: "review_required",
        sourceReference: "v0.1 state delivery error baseline",
      }),
      stateComponentExample({
        id: "toast-task-completed",
        label: localized("Task completed", "任务完成"),
        meaning: localized("User receives immediate completion feedback.", "用户收到即时完成反馈。"),
        userMessage: localized("Bridge via eBridge verified. +120 pts added.", "通过 eBridge bridge 已验证，+120 积分已加入。"),
        nextAction: localized("Continue to the next unlocked task.", "继续下一个已解锁任务。"),
        ownerRole: "project_owner",
        readiness: "covered",
        sourceReference: "v0.1 state delivery screen 15",
      }),
      stateComponentExample({
        id: "toast-verification-failed",
        label: localized("Verification failed", "验证失败"),
        meaning: localized("Failure feedback includes a recovery path.", "失败反馈包含恢复路径。"),
        userMessage: localized("Verification failed. Retry after completing bridge or swap.", "验证失败。完成 bridge 或 swap 后重试。"),
        nextAction: localized("Retry verification or open the required task.", "重新验证或打开必需任务。"),
        ownerRole: "project_owner",
        readiness: "review_required",
        sourceReference: "v0.1 state delivery screen 15",
      }),
      stateComponentExample({
        id: "toast-review-required",
        label: localized("Review required", "需要审核"),
        meaning: localized("Notification tells the owner when human review is required.", "通知告诉项目方何时需要人工审核。"),
        userMessage: localized("Manual review required before this result can affect eligibility.", "该结果影响资格前需要人工审核。"),
        nextAction: localized("Open review queue and inspect evidence.", "打开审核队列并检查证据。"),
        ownerRole: "internal_operator",
        readiness: "review_required",
        sourceReference: "Admin review center",
      }),
    ],
  }),
  stateComponentFamily({
    id: "blocked_publish",
    label: localized("Blocked Publish", "发布阻断"),
    description: localized(
      "Publish blockers prevent unsafe launch, export, contract, or reward responsibility drift.",
      "发布阻断防止不安全上线、导出、合约或奖励责任漂移。",
    ),
    ownerRole: "contract_reviewer",
    sourceReference: "Publish readiness and contract review gates",
    examples: [
      stateComponentExample({
        id: "publish-reward-provider-missing",
        label: localized("Reward provider missing", "奖励提供方缺失"),
        meaning: localized("Campaign cannot publish without explicit project-owned reward responsibility.", "缺少明确项目方奖励责任时活动不能发布。"),
        userMessage: localized("Reward provider and disclaimer are required before publish.", "发布前必须填写奖励提供方和免责声明。"),
        nextAction: localized("Add project reward provider and confirm Campaign OS does not distribute rewards.", "补充项目奖励提供方，并确认 Campaign OS 不发奖。"),
        ownerRole: "project_owner",
        readiness: "blocked",
        sourceReference: "Reward disclaimer review gate",
      }),
      stateComponentExample({
        id: "publish-disclaimer-missing",
        label: localized("Disclaimer missing", "免责声明缺失"),
        meaning: localized("Localized reward disclaimer blocks publish until reviewed.", "本地化奖励免责声明未审核前阻断发布。"),
        userMessage: localized("Reward disclaimer is missing and blocks publish.", "奖励免责声明缺失，已阻断发布。"),
        nextAction: localized("Review localized disclaimer and keep reward distribution project-owned.", "审核本地化免责声明，并保持奖励发放由项目方负责。"),
        ownerRole: "project_owner",
        readiness: "blocked",
        sourceReference: "Reward disclaimer review gate",
      }),
      stateComponentExample({
        id: "publish-contract-claim-review",
        label: localized("Contract claim review", "合约 claim 审核"),
        meaning: localized("Contract claim mode needs high-impact manual review.", "合约 claim 模式需要高影响人工审核。"),
        userMessage: localized("Contract claim mode requires admin approval before publish intent.", "合约 claim 模式在发布意图前需要管理员批准。"),
        nextAction: localized("Switch to Off-chain MVP or complete contract reviewer approval.", "切换到 Off-chain MVP，或完成合约审核人批准。"),
        ownerRole: "contract_reviewer",
        readiness: "blocked",
        sourceReference: "Contract Impact Review",
      }),
    ],
  }),
];
...
export const createContractClaimThreatModelApprovalReadiness = ({
  ...
  const sections: ContractClaimThreatModelSection[] = [
    ...
    contractClaimThreatModelSection({
      ...
      evidenceRequired: securityEvidence("double-claim-replay-prevention"),
      reviewQuestion: localized(
        "How are duplicate claims, replay attempts, indexer drift, and idempotency failures detected and stopped?",
        "如何检测并阻止重复 claim、replay、indexer 漂移和幂等失败？",
        "如何偵測並阻止重複 claim、replay、indexer 漂移和冪等失敗？",
      ),
      mitigation: localized(
        "Add replay, nonce/idempotency, and reconciliation evidence to the security package.",
        "将 replay、nonce/幂等和对账证据加入安全 package。",
        "將 replay、nonce/冪等和對帳證據加入安全 package。",
      ),
      residualRisk: localized(
        "Duplicate-claim abuse is a blocked high-risk section until replay analysis exists.",
        "replay 分析存在前，重复 claim 滥用是已阻断的 high-risk section。",
        "replay 分析存在前，重複 claim 濫用是已阻斷的 high-risk section。",
      ),
      nextAction: securityNextAction("double-claim-replay-prevention"),
      boundary: contractClaimThreatModelApprovalBoundary,
      blocksThreatModelApproval: true,
    }),
    contractClaimThreatModelSection({
      id: "pause-dispute-abuse",
      label: localized("Pause and dispute abuse", "暂停与争议滥用", "暫停與爭議濫用"),
      state: "review_required",
      ownerRole: "internal_operator",
      ...
      riskLevel: "medium",
      dependency: securityDependency("pause-dispute-semantics"),
      evidenceRequired: securityEvidence("pause-dispute-semantics"),
      reviewQuestion: localized(
        "Can pause, dispute, failed-claim, or manual review flows be abused to block valid participants or force payouts?",
        "暂停、争议、失败 claim 或人工审核流程是否可能被滥用来阻断有效用户或强制 payout？",
        "暫停、爭議、失敗 claim 或人工審核流程是否可能被濫用來阻斷有效使用者或強制 payout？",
      ),
      mitigation: localized(
        "Document runbook ownership and dispute decision boundaries before approval.",
        "批准前记录 runbook 归属和争议决策边界。",
        "批准前記錄 runbook 歸屬和爭議決策邊界。",
      ),
      residualRisk: localized(
        "Operational abuse remains possible until runbook approval is complete.",
        "runbook 批准完成前仍存在运营滥用风险。",
        "runbook 批准完成前仍存在營運濫用風險。",
      ),
      nextAction: securityNextAction("pause-dispute-semantics"),
      boundary: contractClaimThreatModelApprovalBoundary,
      blocksThreatModelApproval: true,
    }),
    contractClaimThreatModelSection({
      id: "rollback-failure-abuse",
      label: localized("Rollback failure abuse", "回滚失败滥用", "回滾失敗濫用"),
      state: "review_required",
      ownerRole: "internal_operator",
      sourceSurface: securityItem("rollback-behavior")?.sourceSurface ?? localized("Post-campaign closeout", "活动后 closeout", "活動後 closeout"),
      riskLevel: "medium",
      dependency: securityDependency("rollback-behavior"),
      evidenceRequired: securityEvidence("rollback-behavior"),
      reviewQuestion: localized(
        "What happens when deployment, proof publication, claim UI, or reconciliation must be rolled back?",
        "部署、proof 发布、claim UI 或对账需要回滚时会发生什么？",
        "部署、proof 發布、claim UI 或對帳需要回滾時會發生什麼？",
      ),
      mitigation: localized(
        "Require rollback behavior and failed-claim handling evidence before threat model approval.",
        "威胁模型批准前要求回滚行为和失败 claim 处理证据。",
        "威脅模型批准前要求回滾行為和失敗 claim 處理證據。",
      ),
      residualRisk: localized(
        "Rollback failure is not fully mitigated while closeout runbooks are still under review.",
        "closeout runbook 仍在审核时，回滚失败未被完全缓解。",
        "closeout runbook 仍在審核時，回滾失敗未被完全緩解。",
      ),
      nextAction: securityNextAction("rollback-behavior"),
      boundary: contractClaimThreatModelApprovalBoundary,
      blocksThreatModelApproval: true,
    }),
    contractClaimThreatModelSection({
      id: "mitigation-coverage",
      label: localized("Mitigation coverage", "缓解覆盖度", "緩解覆蓋度"),
      state: "blocked",
      ownerRole: "contract_reviewer",
      sourceSurface: localized("Security Review Readiness", "安全审核 Readiness", "安全審核 Readiness"),
      riskLevel: "high",
      ...
      evidenceRequired: localized(
        "Mitigation matrix is required for all blocked threat model sections.",
        "所有已阻断威胁模型 section 都需要 mitigation matrix。",
        "所有已阻斷威脅模型 section 都需要 mitigation matrix。",
      ),
      reviewQuestion: localized(
        "Do mitigations cover every high-risk abuse path without enabling execution?",
        "缓解措施是否覆盖所有 high-risk 滥用途径且不启用执行？",
        "緩解措施是否覆蓋所有 high-risk 濫用途徑且不啟用執行？",
      ),
      mitigation: localized(
        "Map every blocked section to owner, evidence, review question, residual risk, and next action.",
        "将每个已阻断 section 映射到 owner、evidence、review question、residual risk 和 next action。",
        "將每個已阻斷 section 映射到 owner、evidence、review question、residual risk 和 next action。",
      ),
      residualRisk: localized(
        "Mitigation coverage is incomplete while blocked security items remain.",
        "仍有 blocked security items 时，缓解覆盖度不完整。",
        "仍有 blocked security items 時，緩解覆蓋度不完整。",
      ),
      nextAction: securityReviewReadiness.summary.topNextAction,
      boundary: contractClaimThreatModelApprovalBoundary,
      blocksThreatModelApproval: true,
    ...
    contractClaimThreatModelSection({
      ...
      evidenceRequired: localized(
        "Residual risk acceptance from security, admin, custody/legal, and contract reviewer owners is absent.",
        "缺少安全、admin、托管/法律和合约审核 owner 的残余风险接受记录。",
        "缺少安全、admin、託管/法律和合約審核 owner 的殘餘風險接受記錄。",
      ),
      reviewQuestion: localized(
        "Which residual risks are explicitly accepted, rejected, or deferred before any claim execution can be approved?",
        "任何 claim 执行获批前，哪些残余风险被明确接受、拒绝或延期？",
        "任何 claim 執行獲批前，哪些殘餘風險被明確接受、拒絕或延期？",
      ),
      mitigation: localized(
        "Require named owner acceptance and keep claim execution disabled until acceptance exists.",
        "要求具名 owner 接受，并在接受记录存在前保持 claim 执行禁用。",
        "要求具名 owner 接受，並在接受記錄存在前保持 claim 執行停用。",
      ),
      residualRisk: localized(
        "Residual risk is not accepted, so threat model approval is blocked.",
        "残余风险尚未接受，因此威胁模型批准被阻断。",
        "殘餘風險尚未接受，因此威脅模型批准被阻斷。",
      ),
    ...
    contractClaimThreatModelSection({
      ...
      label: localized("Approval evidence", "批准证据", "批准證據"),
      state: "blocked",
      ownerRole: "contract_reviewer",
      sourceSurface: localized("Contract Claim Preapproval Package", "Contract Claim 预批准 Package", "Contract Claim 預批准 Package"),
      riskLevel: "high",
      dependency: localized(
        "Threat model approval requires signed-off evidence from security, admin, custody/legal, audit, and execution approval readiness.",
        "威胁模型批准需要安全、admin、托管/法律、审计和执行批准 readiness 的签核证据。",
        "威脅模型批准需要安全、admin、託管/法律、審計和執行批准 readiness 的簽核證據。",
      ),
      evidenceRequired: localized(
        "Approval evidence package is missing; no approval, export, branch, issue, PR, or mission automation is created.",
        "批准证据 package 缺失；不会创建批准、导出、分支、issue、PR 或 mission 自动化。",
        "批准證據 package 缺失；不會建立批准、匯出、分支、issue、PR 或 mission 自動化。",
      ),
      reviewQuestion: localized(
        "Is every approval evidence item traceable to a read-only source without creating live side effects?",
        "每个批准证据项是否都可追溯到只读来源且不会产生 live side effect？",
        "每個批准證據項是否都可追溯到唯讀來源且不會產生 live side effect？",
      ),
      mitigation: localized(
        "Collect approval evidence manually in the review package; do not generate files or automate workflow from the UI.",
        "在审核 package 中人工收集批准证据；不要从 UI 生成文件或自动化 workflow。",
        "在審核 package 中人工收集批准證據；不要從 UI 生成檔案或自動化 workflow。",
      ),
      residualRisk: localized(
        "Approval evidence absence keeps the threat model unapproved.",
        "批准证据缺失使威胁模型保持未批准。",
        "批准證據缺失使威脅模型保持未批准。",
      ),
      nextAction: localized(
        "Prepare approval evidence as a review artifact only.",
        "仅以审核 artifact 形式准备批准证据。",
        "僅以審核 artifact 形式準備批准證據。",
      ),
      boundary: contractClaimThreatModelApprovalBoundary,
      blocksThreatModelApproval: true,
    }),
    contractClaimThreatModelSection({
      ...
      state: "ready",
      ownerRole: "project_owner",
      sourceSurface: localized("Delivery Acceptance + Contract Review Center", "Delivery Acceptance + 合约审核中心", "Delivery Acceptance + 合約審核中心"),
      riskLevel: "low",
      dependency: localized(
        "Campaign OS current MVP boundary already excludes reward custody and reward distribution.",
        "Campaign OS 当前 MVP 边界已经排除奖励托管和发奖。",
        "Campaign OS 目前 MVP 邊界已經排除獎勵託管和發獎。",
      ),
      evidenceRequired: claimAcceptanceRow?.evidenceSummary ?? noRewardCustodyBoundary,
      reviewQuestion: localized(
        "Does this threat model preserve no-custody and no-distribution as hard boundaries?",
        "此威胁模型是否将不托管和不发奖保留为硬边界？",
        "此威脅模型是否將不託管和不發獎保留為硬邊界？",
      ),
      mitigation: localized(
        "Keep no-custody/no-distribution language in every section and future claim review.",
        "在每个 section 和未来 claim 审核中保留不托管/不发奖表述。",
        "在每個 section 和未來 claim 審核中保留不託管/不發獎表述。",
      ),
      residualRisk: localized(
        "Boundary is ready as a restriction only; it does not approve threat model or claim execution.",
        "边界仅作为限制已就绪；它不批准威胁模型或 claim 执行。",
        "邊界僅作為限制已就緒；它不批准威脅模型或 claim 執行。",
      ),
      nextAction: localized(
        "Preserve this boundary while the threat model remains blocked.",
        "威胁模型保持阻断时保留此边界。",
        "威脅模型保持阻斷時保留此邊界。",
      ),
      boundary: noRewardCustodyBoundary,
...
export const createContractClaimActorApprovalReadiness = ({
  ...
  const actors: ContractClaimActorApprovalRow[] = [
    ...
    contractClaimActorApprovalRow({
      ...
      label: localized("Contract reviewer", "合约审核人", "合約審核人"),
      state: "blocked",
      ownerRole: "contract_reviewer",
      riskLevel: "high",
      responsibility: localized(
        "Contract reviewer owns ABI, role, event, pause, dispute, proof, and actor authority review.",
        "合约审核人负责 ABI、角色、事件、暂停、争议、proof 和角色权限审核。",
        "合約審核人負責 ABI、角色、事件、暫停、爭議、proof 和角色權限審核。",
      ),
      authorityBoundary: localized(
        "Contract reviewer can block or require evidence but cannot execute contract writes, deploy ABI, or approve payouts through this surface.",
        "合约审核人可以阻断或要求证据，但不能通过此 surface 执行合约写入、部署 ABI 或批准 payout。",
        "合約審核人可以阻斷或要求證據，但不能透過此 surface 執行合約寫入、部署 ABI 或批准 payout。",
      ),
      abusePath: localized(
        "Reviewer authority could be bypassed if role scope, emergency controls, or audit handoff are not explicit.",
        "如果角色范围、紧急控制或审计交接不明确，审核人权限可能被绕过。",
        "如果角色範圍、緊急控制或審計交接不明確，審核人權限可能被繞過。",
      ),
      evidenceRequired: gateEvidence("contract-reviewer-approval"),
      residualRisk: localized(
      ...
      nextAction: gateNextAction("contract-reviewer-approval"),
      sourceSurface: localized("Contract Claim Preapproval Package: contract-reviewer-approval", "Contract Claim 预批准 Package：contract-reviewer-approval", "Contract Claim 預批准 Package：contract-reviewer-approval"),
      blocksActorApproval: true,
    }),
    contractClaimActorApprovalRow({
      id: "verifier",
      label: localized("Verifier", "验证方", "驗證方"),
      state: "review_required",
      ownerRole: "contract_reviewer",
      riskLevel: "medium",
      responsibility: localized(
        "Verifier checks eligibility proof lineage, root scope, and duplicate-claim safeguards before future execution approval.",
        "验证方在未来执行批准前检查资格 proof lineage、root scope 和重复 claim 防护。",
        "驗證方在未來執行批准前檢查資格 proof lineage、root scope 和重複 claim 防護。",
      ),
      authorityBoundary: localized(
        "Verifier cannot write roots, sign wallet actions, call providers, or execute claims in this package.",
        "验证方不能在此 package 中写入 root、钱包签名、调用 provider 或执行 claim。",
        "驗證方不能在此 package 中寫入 root、錢包簽名、呼叫 provider 或執行 claim。",
      ),
      abusePath: localized(
        "Weak verifier scope could allow tampered proof, replay, or drift from exported-list lineage.",
        "验证方范围薄弱可能允许篡改 proof、replay 或偏离导出名单 lineage。",
        "驗證方範圍薄弱可能允許竄改 proof、replay 或偏離匯出名單 lineage。",
      ),
    ...
    contractClaimActorApprovalRow({
      id: "exporter",
      label: localized("Exporter", "导出方", "匯出方"),
      state: "blocked",
      ownerRole: "internal_operator",
      riskLevel: "high",
      responsibility: localized(
        "Exporter may prepare review evidence and list lineage but must not generate claim export files or storage writes in this mission.",
        "导出方可以准备审核证据和名单 lineage，但本 mission 不得生成 claim 导出文件或存储写入。",
        "匯出方可以準備審核證據和名單 lineage，但本 mission 不得生成 claim 匯出檔案或儲存寫入。",
      ),
      authorityBoundary: localized(
        "Exporter has no export generation, storage write, contract root write, branch, issue, PR, or mission automation authority.",
        "导出方没有导出生成、存储写入、合约 root 写入、分支、issue、PR 或 mission 自动化权限。",
        "匯出方沒有匯出生成、儲存寫入、合約 root 寫入、分支、issue、PR 或 mission 自動化權限。",
      ),
      abusePath: localized(
        "Export authority could leak winner lists, imply eligibility root publication, or create unreviewed claim artifacts.",
        "导出权限可能泄露 winner list、暗示资格 root 发布，或创建未经审核的 claim artifacts。",
        "匯出權限可能洩露 winner list、暗示資格 root 發布，或建立未經審核的 claim artifacts。",
      ),
    ...
    contractClaimActorApprovalRow({
      id: "pauser",
      label: localized("Pauser", "暂停操作方", "暫停操作方"),
      state: "review_required",
      ownerRole: "internal_operator",
      riskLevel: "medium",
      responsibility: localized(
        "Pauser owns future emergency pause, dispute, failed-claim, rollback, and manual review boundaries.",
        "暂停操作方负责未来紧急暂停、争议、失败 claim、回滚和人工审核边界。",
        "暫停操作方負責未來緊急暫停、爭議、失敗 claim、回滾和人工審核邊界。",
      ),
      authorityBoundary: localized(
        "Pauser can document runbook readiness only and cannot pause a live contract or execute rollback from this UI.",
        "暂停操作方只能记录 runbook readiness，不能从此 UI 暂停真实合约或执行回滚。",
        "暫停操作方只能記錄 runbook readiness，不能從此 UI 暫停真實合約或執行回滾。",
      ),
      abusePath: localized(
        "Pause or dispute power could block valid participants or force payout decisions if ownership is unclear.",
        "如果归属不清，暂停或争议权限可能阻断有效参与者或强制 payout 决策。",
        "如果歸屬不清，暫停或爭議權限可能阻斷有效參與者或強制 payout 決策。",
      ),
    ...
    contractClaimActorApprovalRow({
      id: "external-auditor",
      label: localized("External auditor", "外部审计方", "外部審計方"),
      state: "blocked",
      ownerRole: "contract_reviewer",
      riskLevel: "high",
      responsibility: localized(
        "External auditor reviews claim contract code, root/proof verification, emergency controls, and actor authority.",
        "外部审计方审核 claim contract 代码、root/proof 验证、紧急控制和角色权限。",
        "外部審計方審核 claim contract 程式碼、root/proof 驗證、緊急控制和角色權限。",
      ),
      authorityBoundary: localized(
        "External auditor supplies review evidence only and cannot approve or execute claim-mode operations in Campaign OS.",
        "外部审计方只提供审核证据，不能在 Campaign OS 中批准或执行 claim-mode 操作。",
        "外部審計方只提供審核證據，不能在 Campaign OS 中批准或執行 claim-mode 操作。",
      ),
      abusePath: localized(
        "Missing audit acceptance could leave high-risk actor authority and emergency controls unreviewed.",
        "缺少审计接受可能导致 high-risk 角色权限和紧急控制未经审核。",
        "缺少審計接受可能導致 high-risk 角色權限和緊急控制未經審核。",
      ),
      evidenceRequired: gateEvidence("external-audit"),
      residualRisk: localized(
      ...
      nextAction: gateNextAction("external-audit"),
      sourceSurface: localized("Contract Claim Preapproval Package: external-audit", "Contract Claim 预批准 Package：external-audit", "Contract Claim 預批准 Package：external-audit"),
      blocksActorApproval: true,
    }),
    contractClaimActorApprovalRow({
      id: "no-custody-no-distribution-boundary",
      ...
      riskLevel: "low",
      responsibility: localized(
        "Boundary preserves that Campaign OS does not custody rewards, distribute rewards, execute payouts, or sign wallet actions.",
        "边界保留 Campaign OS 不托管奖励、不发奖、不执行 payout、不进行钱包签名。",
        "邊界保留 Campaign OS 不託管獎勵、不發獎、不執行 payout、不進行錢包簽名。",
      ),
      authorityBoundary: localized(
        "This boundary is a restriction only; it grants no actor approval, claim execution, custody, payout, or distribution authority.",
        "此边界仅为限制；不授予任何角色批准、claim 执行、托管、payout 或发奖权限。",
        "此邊界僅為限制；不授予任何角色批准、claim 執行、託管、payout 或發獎權限。",
      ),
      abusePath: localized(
        "If this boundary is weakened, operators could infer that Campaign OS owns custody or distribution responsibility.",
        "如果此边界被弱化，运营方可能推断 Campaign OS 拥有托管或发奖责任。",
        "如果此邊界被弱化，營運方可能推斷 Campaign OS 擁有託管或發獎責任。",
      ),
      evidenceRequired: claimAcceptanceRow?.evidenceSummary ?? noRewardCustodyBoundary,
      residualRisk: localized(
        "Boundary is ready as a restriction only; it does not approve actors, threat model, or claim execution.",
        "边界仅作为限制已就绪；它不批准角色、威胁模型或 claim 执行。",
        "邊界僅作為限制已就緒；它不批准角色、威脅模型或 claim 執行。",
      ),
      nextAction: localized(
        "Preserve no-custody and no-distribution in every actor approval review.",
        "在每次角色批准审核中保留不托管和不发奖。",
        "在每次角色批准審核中保留不託管和不發獎。",
      ),
      sourceSurface: localized("Delivery Acceptance + Post-campaign closeout", "Delivery Acceptance + 活动后 closeout", "Delivery Acceptance + 活動後 closeout"),
      blocksActorApproval: true,
    }),
  ];
...
export const createContractClaimCustodyLegalReadiness = ({
  ...
  const items: ContractClaimCustodyLegalItem[] = [
    ...
    contractClaimCustodyLegalItem({
      ...
      sourceGateId: "custody-legal-approval",
      dependency: localized(
        "Legal review must confirm jurisdiction, campaign eligibility, reward classification, tax, and sanctions assumptions.",
        "法律审核必须确认司法辖区、活动资格、奖励分类、税务和制裁假设。",
        "法律審核必須確認司法轄區、活動資格、獎勵分類、稅務和制裁假設。",
      ),
      evidenceRequired: localized(
        "Jurisdiction compliance evidence is absent for claim mode.",
        "claim mode 缺少司法辖区合规证据。",
        "claim mode 缺少司法轄區合規證據。",
      ),
      blockingReason: localized(
        "Jurisdiction compliance is not approved, so custody/legal approval stays blocked.",
        "司法辖区合规尚未批准，因此托管/法律批准保持阻断。",
        "司法轄區合規尚未批准，因此託管/法律批准保持阻斷。",
      ),
      nextAction: localized(
        "Attach jurisdiction compliance review before custody/legal approval can pass.",
        "先附加司法辖区合规审核，再通过托管/法律批准。",
        "先附加司法轄區合規審核，再通過託管/法律批准。",
      ),
      sourceSurface: localized("Legal review", "法律审核", "法律審核"),
      boundary: contractClaimCustodyLegalBoundary,
      blocksCustodyLegalApproval: true,
    }),
    contractClaimCustodyLegalItem({
...
const v01AcceptanceRows = (
  ...
  return [
    ...
    deliveryAcceptanceRow({
      ...
      nextMissionAction: localized(
        "Keep production participation readiness gated through the live wallet provider evidence path before enabling real verification providers.",
        "启用真实验证 provider 前，继续通过 live wallet provider evidence 路径门禁生产参与 readiness。",
        "啟用真實驗證 provider 前，繼續透過 live wallet provider evidence 路徑門禁生產參與 readiness。",
      ),
    }),
    deliveryAcceptanceRow({
...
const walletProviderEvidenceApprovalAcceptanceSummary = (
  ...
  const topFailedRule = releaseReadiness.summary.topFailedRuleId ?? "none";

  if (releaseReadiness.summary.ready) {
    return localized(
      `Wallet Provider Evidence Release Readiness has approved all ${releaseReadiness.summary.requiredScenarios}/${releaseReadiness.summary.requiredScenarios} required scenarios; live execution remains separately gated by the non-live boundary.`,
      `Wallet Provider Evidence Release Readiness 已批准全部 ${releaseReadiness.summary.requiredScenarios}/${releaseReadiness.summary.requiredScenarios} 个必需场景；真实执行仍受非实时边界单独门禁控制。`,
      `Wallet Provider Evidence Release Readiness 已批准全部 ${releaseReadiness.summary.requiredScenarios}/${releaseReadiness.summary.requiredScenarios} 個必需場景；真實執行仍受非即時邊界單獨門禁控制。`,
    );
  }

  return localized(
    `Wallet Provider Evidence Release Readiness has approved ${releaseReadiness.summary.approvedRequiredScenarios}/${releaseReadiness.summary.requiredScenarios} required scenarios; ${releaseReadiness.summary.releaseBlockers} release blockers and top failed rule ${topFailedRule} keep production onboarding blocked.`,
    `Wallet Provider Evidence Release Readiness 已批准 ${releaseReadiness.summary.approvedRequiredScenarios}/${releaseReadiness.summary.requiredScenarios} 个必需场景；${releaseReadiness.summary.releaseBlockers} 个发布阻断与 top failed rule ${topFailedRule} 让生产 onboarding 继续阻断。`,
    `Wallet Provider Evidence Release Readiness 已批准 ${releaseReadiness.summary.approvedRequiredScenarios}/${releaseReadiness.summary.requiredScenarios} 個必需場景；${releaseReadiness.summary.releaseBlockers} 個發布阻斷與 top failed rule ${topFailedRule} 讓生產 onboarding 繼續阻斷。`,
  );
};
...
const v02AcceptanceRows = (
  ...
  return [
    ...
    deliveryAcceptanceRow({
      ...
      status: liveWalletProviderReady ? "proven" : "needs_live_evidence",
      severity: liveWalletProviderReady ? "high" : "critical",
      ownerRole: "internal_operator",
      launchBlocking: !liveWalletProviderReady,
      boundary: walletProviderEvidenceBoundary,
      evidenceSurface: walletProviderEvidenceApprovalAcceptanceSurface(releaseReadiness),
      evidenceSummary: walletProviderEvidenceApprovalAcceptanceSummary(releaseReadiness),
      nextMissionAction: releaseReadiness?.nextAction ?? localized(
    ...
    deliveryAcceptanceRow({
      ...
      title: localized("P1 runtime locale expansion is covered", "P1 运行时语言扩展已覆盖", "P1 執行時語言擴展已覆蓋"),
      status: "proven",
      severity: "low",
      ownerRole: "project_owner",
      evidenceSurface: localized("P1 locale expansion readiness", "P1 语言扩展 readiness", "P1 語言擴展 readiness"),
      evidenceSummary: localized(
        "ja-JP, ko-KR, vi-VN, id-ID, tr-TR, and es-ES are active with English fallback; reviewed locale copy remains separate from runtime activation.",
        "ja-JP、ko-KR、vi-VN、id-ID、tr-TR 与 es-ES 均已激活并使用英文 fallback；已审核语言文案仍与运行时激活分开管理。",
        "ja-JP、ko-KR、vi-VN、id-ID、tr-TR 與 es-ES 均已啟用並使用英文 fallback；已審核語言文案仍與執行時啟用分開管理。",
      ),
      nextMissionAction: localized(
        "Complete reviewed Japanese, Korean, Vietnamese, Indonesian, Turkish, and Spanish copy before claiming full localization.",
        "声明完整本地化前先完成已审核日文、韩文、越南语、印尼语、土耳其语与西班牙语文案。",
        "宣稱完整本地化前先完成已審核日文、韓文、越南語、印尼語、土耳其語與西班牙語文案。",
      ),
    }),
  ];
};
...
const createProductDeliveryChecklistItems = (): DeliveryChecklistItem[] => {
  ...
  return [
    ...
    deliveryChecklistItem({
      ...
      evidence: localized(
        "Runtime support covers en-US, zh-CN, zh-TW, and all P1 fallback locales: ja-JP, ko-KR, vi-VN, id-ID, tr-TR, and es-ES.",
        "运行时已覆盖 en-US、zh-CN、zh-TW 以及全部 P1 fallback 语言：ja-JP、ko-KR、vi-VN、id-ID、tr-TR 与 es-ES。",
      ),
      nextAction: localized(
        "Keep reviewed locale copy separate before claiming full localization.",
        "声明完整本地化前继续将已审核语言文案分开管理。",
      ),
    ...
    deliveryChecklistItem({
      ...
      evidence: localized(
        "Off-chain MVP, V2 companion, and contract claim modes are visible with claim-mode disabled and future approval-gated.",
        "Off-chain MVP、V2 companion 与 contract claim 模式可见，claim mode 已禁用并由未来审批门禁控制。",
      ),
      nextAction: localized(
        "Keep claim mode disabled in MVP and route future activation through contract reviewer approval.",
        "MVP 保持 claim mode 禁用，未来启用必须经过合约审核人批准。",
      ),
    ...
    deliveryChecklistItem({
      ...
      label: localized("Additional P1 runtime locale expansion is covered", "额外 P1 运行时语言扩展已覆盖"),
      status: "covered",
      ownerRole: "project_owner",
      surface: localized("Locale governance", "语言治理"),
      evidence: localized(
        "The v0.2 MVP locales plus ja-JP, ko-KR, vi-VN, id-ID, tr-TR, and es-ES runtime activation are covered with English fallback.",
        "v0.2 MVP 语言以及 ja-JP、ko-KR、vi-VN、id-ID、tr-TR、es-ES 运行时激活均已使用英文 fallback 覆盖。",
      ),
      nextAction: localized(
        "Use the P1 locale readiness matrix to track reviewed copy before claiming full localization.",
        "使用 P1 语言 readiness 矩阵跟踪已审核文案，再声明完整本地化。",
        "使用 P1 語言 readiness 矩陣追蹤已審核文案，再宣稱完整本地化。",
      ),
...
const createArchitectureDeliveryChecklistItems = (): DeliveryChecklistItem[] => {
  ...
  return [
    ...
    deliveryChecklistItem({
      ...
      evidence: localized(
        "Mode options are visible and contract claim is disabled for MVP as a high-impact future gate.",
        "模式选项可见，contract claim 在 MVP 中禁用，并作为高影响未来门禁处理。",
      ),
...
const createUiDeliveryChecklistItems = (): DeliveryChecklistItem[] => {
  ...
  return [
    ...
    deliveryChecklistItem({
      ...
      nextAction: localized(
        "Keep claim-mode rows disabled until separate security, custody/legal, audit, and admin approval.",
        "在单独的安全、托管/法律、审计与管理员批准前，保持 claim-mode 行为禁用。",
      ),
...
const createContractDeliveryChecklistItems = (): DeliveryChecklistItem[] => {
  ...
  return [
    ...
    deliveryChecklistItem({
      ...
      evidence: localized(
        "Reward custody is an accepted MVP exclusion. Campaign OS exports winners only; final reward distribution is handled by the campaign project.",
        "奖励托管是已接受的 MVP 排除项。Campaign OS 只导出 winners；最终奖励发放由活动项目方处理。",
        "獎勵託管是已接受的 MVP 排除項。Campaign OS 只匯出 winners；最終獎勵發放由活動專案方處理。",
      ),
      nextAction: localized(
        "Keep reward custody outside Campaign OS until separate security, custody/legal, external audit, and admin approval exists.",
        "在单独的安全、托管/法律、外部审计与管理员批准前，将奖励托管保持在 Campaign OS 外。",
        "在單獨的安全、託管/法律、外部審計與管理員批准前，將獎勵託管保持在 Campaign OS 外。",
      ),
...
const createQaDeliveryChecklistItems = (
  ...
  return [
    ...
    deliveryChecklistItem({
      ...
      surface: localized("Contract Impact Review", "合约影响审核"),
      evidence: localized(
        "Contract claim execution is disabled for MVP and requires future security, custody/legal, external audit, and admin approval.",
        "MVP 禁用 Contract claim 执行，未来需要安全、托管/法律、外部审计与管理员批准。",
        "MVP 停用 Contract claim 執行，未來需要安全、託管/法律、外部審計與管理員批准。",
      ),
      nextAction: localized(
        "Keep claim mode disabled in MVP and route any future request through contract reviewer approval.",
        "MVP 保持 claim mode 禁用，任何未来请求都必须经过合约审核人批准。",
        "MVP 保持 claim mode 停用，任何未來請求都必須經過合約審核人批准。",
      ),
...
const forestNftTaskIntentLabels: Record<ForestNftTaskIntentId, LocalizedText> = {
  ...
  "forest-nft-holder-evidence": localized(
    "Forest NFT holder evidence",
    "Forest NFT holder 证据",
    "Forest NFT holder evidence",
  ),
  "forest-nft-trade-listing-review": localized(
    "Forest NFT trade/listing review",
    "Forest NFT trade/listing 审核",
    "Forest NFT trade/listing review",
  ),
  "forest-holder-leaderboard-review": localized(
...
const forestNftTaskIntentDescriptions: Record<ForestNftTaskIntentId, LocalizedText> = {
  "forest-nft-mint-readiness": localized(
    "Review whether a Forest NFT mint task can be represented from seeded campaign metadata before publish.",
    "发布前审核 Forest NFT mint 任务是否可由 seeded 活动 metadata 表达。",
    "Review whether a Forest NFT mint task can be represented from seeded campaign metadata before publish.",
  ),
  "forest-nft-holder-evidence": localized(
    "Review holder snapshot evidence before using Forest NFT ownership in campaign scoring.",
    "在活动计分中使用 Forest NFT 持有前，先审核 holder snapshot 证据。",
    "Review holder snapshot evidence before using Forest NFT ownership in campaign scoring.",
  ),
  "forest-nft-trade-listing-review": localized(
    "Review marketplace trade or listing evidence before counting Forest marketplace actions.",
    "计入 Forest marketplace 行为前，先审核 trade 或 listing 证据。",
    "Review marketplace trade or listing evidence before counting Forest marketplace actions.",
  ),
  "forest-holder-leaderboard-review": localized(
    "Review holder leaderboard ownership before ranking campaign users by Forest NFT status.",
    "按 Forest NFT 状态为活动用户排名前，先审核 holder 排行榜归属。",
    "Review holder leaderboard ownership before ranking campaign users by Forest NFT status.",
  ),
};

...
const activatedP1RuntimeLocales = new Set<P1LocaleCode>(["ja-JP", "ko-KR", "vi-VN", "id-ID"]);

const p1LocaleExpansionBoundary = localized(
  "P1 locale rows are backlog planning except ja-JP, ko-KR, vi-VN, and id-ID, which are runtime-active with English fallback until reviewed locale copy is complete.",
  "除已激活且在语言审核完成前使用英文 fallback 的 ja-JP、ko-KR、vi-VN 与 id-ID 外，P1 语言行仅用于 backlog 规划。",
  "除已啟用且在語言審核完成前使用英文 fallback 的 ja-JP、ko-KR、vi-VN 與 id-ID 外，P1 語言行僅用於 backlog 規劃。",
);
...
const p1LocaleActivationBoundary = localized(
  "ja-JP, ko-KR, vi-VN, and id-ID are runtime-active with English fallback until reviewed locale copy is complete. Other P1 locales remain review-only backlog.",
  "ja-JP、ko-KR、vi-VN 与 id-ID 已激活运行时，并在语言审核完成前使用英文 fallback。其他 P1 语言仍为只读审核 backlog。",
  "ja-JP、ko-KR、vi-VN 與 id-ID 已啟用執行時，並在語言審核完成前使用英文 fallback。其他 P1 語言仍為只讀審核 backlog。",
);
...
const v02AcceptanceRows = (
  ...
  return [
    ...
    deliveryAcceptanceRow({
      ...
      evidenceSummary: localized(
        "ja-JP, ko-KR, vi-VN, and id-ID are active with English fallback; tr-TR and es-ES remain backlog rows.",
        "ja-JP、ko-KR、vi-VN 与 id-ID 已激活并使用英文 fallback；tr-TR 与 es-ES 仍作为 backlog rows 跟踪。",
        "ja-JP、ko-KR、vi-VN 與 id-ID 已啟用並使用英文 fallback；tr-TR 與 es-ES 仍作為 backlog rows 跟蹤。",
      ),
      nextMissionAction: localized(
        "Complete reviewed Japanese, Korean, Vietnamese, and Indonesian copy before claiming full localization; keep remaining P1 locales queued for separate activation missions.",
        "声明完整本地化前先完成已审核日文、韩文、越南语与印尼语文案；其余 P1 语言继续排队等待单独激活 mission。",
        "宣稱完整本地化前先完成已審核日文、韓文、越南語與印尼語文案；其餘 P1 語言繼續排隊等待單獨啟用 mission。",
      ),
    }),
  ];
};
...
const createProductDeliveryChecklistItems = (): DeliveryChecklistItem[] => {
  ...
  return [
    ...
    deliveryChecklistItem({
      ...
      nextAction: localized(
        "Keep remaining P1 locales such as tr-TR and es-ES deferred to separate activation missions.",
        "将 tr-TR、es-ES 等剩余 P1 语言继续后置到单独激活 mission。",
      ),
    ...
    deliveryChecklistItem({
      ...
      surface: localized("Locale governance", "语言治理"),
      evidence: localized(
        "The v0.2 MVP locales plus ja-JP, ko-KR, vi-VN, and id-ID runtime activation are covered; remaining P1 locales tr-TR and es-ES remain future scope.",
        "v0.2 MVP 语言以及 ja-JP、ko-KR、vi-VN、id-ID 运行时激活已覆盖；剩余 P1 语言 tr-TR、es-ES 仍为未来范围。",
      ),
      nextAction: localized(

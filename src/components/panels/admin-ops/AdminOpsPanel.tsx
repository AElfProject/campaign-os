...
export const AdminOpsPanel = ({
  ...
  const contractClaimSecurityReviewSummary = [
    {
      id: "total",
      label: copy.totalItems,
      value: contractClaimSecurityReviewReadiness.summary.totalItems,
      state: "ready" as const,
    },
    {
      id: "blocked",
      label: copy.blockedItems,
      value: contractClaimSecurityReviewReadiness.summary.blockedItems,
      state: contractClaimSecurityReviewReadiness.summary.blockedItems > 0 ? "blocker" as const : "ready" as const,
    },
    {
      id: "review-required",
      label: copy.reviewRequiredItems,
      value: contractClaimSecurityReviewReadiness.summary.reviewRequiredItems,
      state: contractClaimSecurityReviewReadiness.summary.reviewRequiredItems > 0
        ? "warning" as const
        : "ready" as const,
    },
    {
      ...
      value: contractClaimSecurityReviewReadiness.summary.readyItems,
      state: "ready" as const,
    },
    {
      id: "approval-blocked",
      label: copy.approvalBlocked,
      value: contractClaimSecurityReviewReadiness.summary.approvalBlocked ? copy.blocked : copy.readyActions,
      state: contractClaimSecurityReviewReadiness.summary.approvalBlocked ? "blocker" as const : "ready" as const,
    },
  ];
  ...
  const contractClaimRewardCustodyApprovalReadiness =
    contractClaimPreapprovalPackage.rewardCustodyApprovalReadiness;
  const contractClaimRewardCustodyApprovalSummary = [
    {
      id: "total",
      label: copy.totalItems,
      value: contractClaimRewardCustodyApprovalReadiness.summary.totalItems,
      state: "ready" as const,
    },
    {
      id: "blocked",
      label: copy.blockedItems,
      value: contractClaimRewardCustodyApprovalReadiness.summary.blockedItems,
      state: contractClaimRewardCustodyApprovalReadiness.summary.blockedItems > 0
        ? "blocker" as const
        : "ready" as const,
    },
    {
      id: "review-required",
      label: copy.reviewRequiredItems,
      value: contractClaimRewardCustodyApprovalReadiness.summary.reviewRequiredItems,
      state: contractClaimRewardCustodyApprovalReadiness.summary.reviewRequiredItems > 0
        ? "warning" as const
        : "ready" as const,
    },
    {
      id: "ready",
      label: copy.readyItems,
      value: contractClaimRewardCustodyApprovalReadiness.summary.readyItems,
      state: "ready" as const,
    },
    {
      id: "launch-blocking",
      label: copy.launchBlockingItems,
      value: contractClaimRewardCustodyApprovalReadiness.summary.launchBlockingItems,
      state: contractClaimRewardCustodyApprovalReadiness.summary.launchBlockingItems > 0
        ? "blocker" as const
        : "ready" as const,
    },
    {
      id: "reward-custody-approval-blocked",
      label: copy.rewardCustodyApprovalBlocked,
      value: contractClaimRewardCustodyApprovalReadiness.summary.rewardCustodyApprovalBlocked
        ? copy.blocked
        : copy.readyActions,
      state: contractClaimRewardCustodyApprovalReadiness.summary.rewardCustodyApprovalBlocked
        ? "blocker" as const
        : "ready" as const,
    },
    {
      id: "reward-custody-enabled",
      label: copy.rewardCustodyEnabled,
      value: contractClaimRewardCustodyApprovalReadiness.summary.rewardCustodyEnabled
        ? copy.readyActions
        : copy.blocked,
      state: contractClaimRewardCustodyApprovalReadiness.summary.rewardCustodyEnabled
        ? "ready" as const
        : "blocker" as const,
    },
  ];
  const contractClaimRewardCustodyApprovalTopItem = contractClaimRewardCustodyApprovalReadiness.items.find(
    (item) => item.id === contractClaimRewardCustodyApprovalReadiness.summary.topItemId,
  ) ?? contractClaimRewardCustodyApprovalReadiness.items[0];
  const contractClaimExecutionApprovalReadiness = contractClaimPreapprovalPackage.executionApprovalReadiness;
  const contractClaimExecutionApprovalSummary = [
    {
      id: "total",
      label: copy.totalItems,
      value: contractClaimExecutionApprovalReadiness.summary.totalItems,
      state: "ready" as const,
    },
    {
      id: "blocked",
      label: copy.blockedItems,
      value: contractClaimExecutionApprovalReadiness.summary.blockedItems,
      state: contractClaimExecutionApprovalReadiness.summary.blockedItems > 0 ? "blocker" as const : "ready" as const,
    },
    {
      id: "review-required",
      label: copy.reviewRequiredItems,
      value: contractClaimExecutionApprovalReadiness.summary.reviewRequiredItems,
      state: contractClaimExecutionApprovalReadiness.summary.reviewRequiredItems > 0
        ? "warning" as const
        : "ready" as const,
    },
    {
      id: "ready",
      label: copy.readyItems,
      value: contractClaimExecutionApprovalReadiness.summary.readyItems,
      state: "ready" as const,
    },
    {
      id: "launch-blocking",
      label: copy.launchBlockingItems,
      value: contractClaimExecutionApprovalReadiness.summary.launchBlockingItems,
      state: contractClaimExecutionApprovalReadiness.summary.launchBlockingItems > 0
        ? "blocker" as const
        : "ready" as const,
    },
    {
      id: "execution-approval-blocked",
      label: copy.executionApprovalBlocked,
      value: contractClaimExecutionApprovalReadiness.summary.executionApprovalBlocked
        ? copy.blocked
        : copy.readyActions,
      state: contractClaimExecutionApprovalReadiness.summary.executionApprovalBlocked
        ? "blocker" as const
        : "ready" as const,
    },
  ];
  ...
  return (
    <div style={{ display: "grid", gap: 18 }}>
      ...
      <section aria-label={copy.contractClaimPreapprovalPackage} style={panelStyle}>
        ...
        <article aria-label={copy.contractClaimExecutionApprovalReadiness} style={cardStyle}>
          <div style={rowStyle}>
            <div style={stackStyle}>
              <p style={labelStyle}>{copy.contractClaimExecutionApprovalBoundary}</p>
              <strong>{copy.contractClaimExecutionApprovalReadiness}</strong>
              <p style={mutedTextStyle}>{copy.contractClaimExecutionApprovalSubtitle}</p>
            </div>
            <PublishStateBadge
              label={contractClaimExecutionApprovalReadiness.summary.executionApprovalBlocked
                ? copy.executionApprovalBlocked
                : copy.readyActions}
              state={contractClaimExecutionApprovalReadiness.summary.executionApprovalBlocked ? "blocker" : "ready"}
            />
          </div>
          <p style={boundaryStyle}>
            {copy.nonLiveBoundary}: {getLocalizedText(contractClaimExecutionApprovalReadiness.boundary, locale)}
          </p>
          <div style={compactGridStyle}>
            {contractClaimExecutionApprovalSummary.map((item) => (
              <article key={item.id} style={cardStyle}>
                <div style={rowStyle}>
                  <p style={labelStyle}>{item.label}</p>
                  <PublishStateBadge label={String(item.value)} state={item.state} />
          ...
          {contractClaimExecutionApprovalTopItem ? (
            <article style={cardStyle}>
              <div style={rowStyle}>
                <div style={stackStyle}>
                  <p style={labelStyle}>{copy.topExecutionApprovalBlocker}</p>
                  <strong>{getLocalizedText(contractClaimExecutionApprovalTopItem.label, locale)}</strong>
                </div>
                <PublishStateBadge
                  label={contractClaimPreapprovalGateLabel(contractClaimExecutionApprovalTopItem.state, copy)}
                  state={contractClaimPreapprovalGateState(contractClaimExecutionApprovalTopItem.state)}
                />
              </div>
              <p style={wrapTextStyle}>
                {copy.ownerRole}: {readableCode(contractClaimExecutionApprovalTopItem.ownerRole)}
              </p>
              <p style={wrapTextStyle}>
                {copy.sourceGate}: {contractClaimExecutionApprovalTopItem.sourceGateId}
              </p>
              <p style={wrapTextStyle}>
                {copy.blockingReason}: {getLocalizedText(contractClaimExecutionApprovalTopItem.blockingReason, locale)}
              </p>
              <p style={wrapTextStyle}>
                {copy.launchImpact}: {getLocalizedText(contractClaimExecutionApprovalTopItem.launchImpact, locale)}
              </p>
              <p style={wrapTextStyle}>
                {copy.nextAction}: {getLocalizedText(contractClaimExecutionApprovalTopItem.nextAction, locale)}
              </p>
            </article>
          ) : null}
          <div style={gridStyle}>
            {contractClaimExecutionApprovalReadiness.items.map((item) => (
              <article key={item.id} style={cardStyle}>
                <div style={rowStyle}>
                  <div style={stackStyle}>
        ...
        <article aria-label={copy.contractClaimRewardCustodyApprovalReadiness} style={cardStyle}>
          <div style={rowStyle}>
            <div style={stackStyle}>
              <p style={labelStyle}>{copy.contractClaimRewardCustodyApprovalBoundary}</p>
              <strong>{copy.contractClaimRewardCustodyApprovalReadiness}</strong>
              <p style={mutedTextStyle}>{copy.contractClaimRewardCustodyApprovalSubtitle}</p>
            </div>
            <PublishStateBadge
              label={contractClaimRewardCustodyApprovalReadiness.summary.rewardCustodyApprovalBlocked
                ? copy.rewardCustodyApprovalBlocked
                : copy.readyActions}
              state={contractClaimRewardCustodyApprovalReadiness.summary.rewardCustodyApprovalBlocked
                ? "blocker"
                : "ready"}
          ...
          <p style={boundaryStyle}>
            {copy.nonLiveBoundary}: {getLocalizedText(contractClaimRewardCustodyApprovalReadiness.boundary, locale)}
          </p>
          <div style={compactGridStyle}>
            {contractClaimRewardCustodyApprovalSummary.map((item) => (
              <article key={item.id} style={cardStyle}>
                <div style={rowStyle}>
                  <p style={labelStyle}>{item.label}</p>
                  <PublishStateBadge label={String(item.value)} state={item.state} />
          ...
          {contractClaimRewardCustodyApprovalTopItem ? (
            <article style={cardStyle}>
              <div style={rowStyle}>
                <div style={stackStyle}>
                  <p style={labelStyle}>{copy.topRewardCustodyApprovalBlocker}</p>
                  <strong>{getLocalizedText(contractClaimRewardCustodyApprovalTopItem.label, locale)}</strong>
                </div>
                <PublishStateBadge
                  label={contractClaimPreapprovalGateLabel(contractClaimRewardCustodyApprovalTopItem.state, copy)}
                  state={contractClaimPreapprovalGateState(contractClaimRewardCustodyApprovalTopItem.state)}
                />
              </div>
              <p style={wrapTextStyle}>
                {copy.ownerRole}: {readableCode(contractClaimRewardCustodyApprovalTopItem.ownerRole)}
              </p>
              <p style={wrapTextStyle}>
                {copy.sourceGate}: {contractClaimRewardCustodyApprovalTopItem.sourceGateId}
              </p>
              <p style={wrapTextStyle}>
                {copy.fundingImpact}: {getLocalizedText(contractClaimRewardCustodyApprovalTopItem.fundingImpact, locale)}
              </p>
              <p style={wrapTextStyle}>
                {copy.approvalImpact}: {getLocalizedText(contractClaimRewardCustodyApprovalTopItem.approvalImpact, locale)}
              </p>
              <p style={wrapTextStyle}>
                {copy.nextAction}: {getLocalizedText(contractClaimRewardCustodyApprovalTopItem.nextAction, locale)}
              </p>
            </article>
          ) : null}
          <div style={gridStyle}>
            {contractClaimRewardCustodyApprovalReadiness.items.map((item) => (
              <article key={item.id} style={cardStyle}>
                <div style={rowStyle}>
                  <div style={stackStyle}>
                    <p style={labelStyle}>{item.id}</p>
                  ...
                  <span style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    <PublishStateBadge
                      ...
                      state={contractClaimPreapprovalGateState(item.state)}
                    />
                    {item.blocksRewardCustodyApproval
                      ? <Badge label={copy.blocksRewardCustodyApproval} tone="warning" />
                      : null}
                  </span>
                </div>
                <p style={wrapTextStyle}>
                ...
                <p style={wrapTextStyle}>
                  {copy.fundingImpact}: {getLocalizedText(item.fundingImpact, locale)}
                </p>
                <p style={wrapTextStyle}>
                  {copy.approvalImpact}: {getLocalizedText(item.approvalImpact, locale)}
                </p>
                <p style={wrapTextStyle}>
                  {copy.nextAction}: {getLocalizedText(item.nextAction, locale)}
                </p>
        ...
        <article aria-label={copy.contractClaimSecurityReviewReadiness} style={cardStyle}>
          <div style={rowStyle}>
            <div style={stackStyle}>
              <p style={labelStyle}>{copy.contractClaimSecurityReviewBoundary}</p>
              <strong>{copy.contractClaimSecurityReviewReadiness}</strong>
              <p style={mutedTextStyle}>{copy.contractClaimSecurityReviewSubtitle}</p>
            </div>
            <PublishStateBadge
              label={contractClaimSecurityReviewReadiness.summary.approvalBlocked
                ? copy.approvalBlocked
                : copy.readyActions}
              state={contractClaimSecurityReviewReadiness.summary.approvalBlocked ? "blocker" : "ready"}
            />
          </div>
          <p style={boundaryStyle}>
            {copy.nonLiveBoundary}: {getLocalizedText(contractClaimSecurityReviewReadiness.boundary, locale)}
          </p>
          <div style={compactGridStyle}>
            {contractClaimSecurityReviewSummary.map((item) => (
              <article key={item.id} style={cardStyle}>
                <div style={rowStyle}>
                  <p style={labelStyle}>{item.label}</p>
                  <PublishStateBadge label={String(item.value)} state={item.state} />
          ...
          {contractClaimSecurityReviewTopItem ? (
            <article style={cardStyle}>
              <div style={rowStyle}>
                <div style={stackStyle}>
                  <p style={labelStyle}>{copy.topSecurityBlocker}</p>
                  <strong>{getLocalizedText(contractClaimSecurityReviewTopItem.label, locale)}</strong>
                </div>
                <PublishStateBadge
                  label={contractClaimPreapprovalGateLabel(contractClaimSecurityReviewTopItem.state, copy)}
                  state={contractClaimPreapprovalGateState(contractClaimSecurityReviewTopItem.state)}
                />
              </div>
              <p style={wrapTextStyle}>
                {copy.evidenceNeeded}: {getLocalizedText(contractClaimSecurityReviewTopItem.evidenceNeeded, locale)}
              </p>
              <p style={wrapTextStyle}>
                {copy.nextAction}: {getLocalizedText(contractClaimSecurityReviewTopItem.nextAction, locale)}
              </p>
            </article>
          ) : null}
          <div style={gridStyle}>
            {contractClaimSecurityReviewReadiness.items.map((item) => (
              <article key={item.id} style={cardStyle}>
                <div style={rowStyle}>
                  <div style={stackStyle}>
                    <p style={labelStyle}>{item.id}</p>
                  ...
                  <span style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    <PublishStateBadge
                      ...
                      state={contractClaimPreapprovalGateState(item.state)}
                    />
                    {item.blocksApproval ? <Badge label={copy.blocksApproval} tone="warning" /> : null}
                  </span>
                </div>
                <p style={wrapTextStyle}>
                  {copy.ownerRole}: {readableCode(item.ownerRole)}
      ...
      <section style={panelStyle}>
        ...
        <div aria-label={copy.exportStorageApprovalReadiness} style={cardStyle}>
          <div style={rowStyle}>
            <div>
              <p style={labelStyle}>{copy.exportStorageApprovalStatus}</p>
              <h4 style={{ fontSize: 18, margin: "2px 0 0" }}>{copy.exportStorageApprovalReadiness}</h4>
              <p style={mutedTextStyle}>{copy.exportStorageApprovalReadinessSubtitle}</p>
            </div>
            <PublishStateBadge
              label={exportStorageFulfillmentApprovalReadiness.summary.approvalBlocked ? copy.approvalBlocked : copy.readyActions}
              state={exportStorageFulfillmentApprovalReadiness.summary.approvalBlocked ? "blocker" : "ready"}
            />
          </div>
          <div style={compactGridStyle}>
            {[
              {
                ...
                value: exportStorageFulfillmentApprovalReadiness.summary.totalChecks,
              },
              {
                label: copy.readyApprovals,
                state: "ready" as const,
                value: exportStorageFulfillmentApprovalReadiness.summary.readyChecks,
              },
              {
                label: copy.reviewRequired,
                state: "warning" as const,
                value: exportStorageFulfillmentApprovalReadiness.summary.reviewRequiredChecks,
              },
              {
                label: copy.blockedApprovals,
                state: "blocker" as const,
                value: exportStorageFulfillmentApprovalReadiness.summary.blockedChecks,
              },
          ...
          {exportStorageApprovalTopCheck ? (
            <article style={{ ...cardStyle, minHeight: 0 }}>
              <div style={rowStyle}>
                <div style={stackStyle}>
                  <p style={labelStyle}>{copy.exportStorageApprovalTopBlocker}</p>
                  <strong>{getLocalizedText(exportStorageApprovalTopCheck.label, locale)}</strong>
                </div>
                <PublishStateBadge
                  label={readableCode(exportStorageApprovalTopCheck.state)}
                  state={exportReadinessState(exportStorageApprovalTopCheck.state)}
                />
              </div>
              <p style={wrapTextStyle}>
                {copy.ownerRole}: {readableCode(exportStorageApprovalTopCheck.ownerRole)}
              </p>
              <p style={wrapTextStyle}>
                {copy.riskLevel}: {exportStorageApprovalTopCheck.riskLevel}
              </p>
              <p style={wrapTextStyle}>
                {copy.exportStorageApprovalRequiredEvidence}: {getLocalizedText(exportStorageApprovalTopCheck.evidenceRequired, locale)}
              </p>
              <p style={wrapTextStyle}>
                {copy.residualRisk}: {getLocalizedText(exportStorageApprovalTopCheck.residualRisk, locale)}
              </p>
              <p style={wrapTextStyle}>
                {copy.exportStorageApprovalNextAction}: {getLocalizedText(exportStorageApprovalTopCheck.nextAction, locale)}
              </p>
            </article>
          ) : null}
          <div style={gridStyle}>
            <article style={{ ...cardStyle, minHeight: 0 }}>
              <p style={labelStyle}>{copy.exportStorageApprovalColumnCoverage}</p>
              <div style={sourceMetricListStyle}>
                {[
                  { label: copy.exactColumnOrder, value: exportStorageFulfillmentApprovalReadiness.columnCoverage.exactOrder },
                  { label: copy.accountType, value: exportStorageFulfillmentApprovalReadiness.columnCoverage.walletTypeIncluded },
                  { label: copy.walletSource, value: exportStorageFulfillmentApprovalReadiness.columnCoverage.walletSourceIncluded },
                  { label: copy.localePreference, value: exportStorageFulfillmentApprovalReadiness.columnCoverage.localePreferenceIncluded },
                  { label: copy.riskFlags, value: exportStorageFulfillmentApprovalReadiness.columnCoverage.riskFlagsIncluded },
                  { label: copy.taskRecords, value: exportStorageFulfillmentApprovalReadiness.columnCoverage.taskEvidenceIncluded },
                ].map((coverage) => (
            ...
            <article style={{ ...cardStyle, minHeight: 0 }}>
              ...
              <p style={boundaryStyle}>
                {getLocalizedText(exportStorageFulfillmentApprovalReadiness.safety.boundary, locale)}
              </p>
            </article>
          </div>
          <div style={gridStyle}>
            {exportStorageFulfillmentApprovalReadiness.checks.map((check) => (
